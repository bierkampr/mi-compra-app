...
const ShoppingListView: React.FC<ShoppingListViewProps> = ({ db, updateAndSync, setPurchaseMode, txt }) => {
  ...
  // NUEVA FUNCIÓN: Limpiar solo lo que no se ha comprado
  const clearPending = async () => {
    if (confirm("¿Borrar todos los productos pendientes de la lista?")) {
  ...
  return (
    <div className="space-y-6 animate-in slide-in-from-right-4 duration-500 pb-10">
      ...
      <div className="relative group">
        ...
        {/* SUGERENCIAS DEL CATÁLOGO SUPABASE */}
      ...
      {db.lista.length > 0 && (
        <div className="flex gap-2.5">
          <button 
            ...
            className="btn-primary flex-[2] py-4 !text-[10px] tracking-widest gap-2 shadow-none"
          >
          ...
          {pendingItems.length > 0 && (
            <button 
                ...
                className="btn-secondary flex-1 !p-0 bg-brand-primary/5 border-brand-primary/10 text-brand-primary hover:bg-brand-primary hover:text-white transition-all"
      ...
      <section className="space-y-4">
        ...
        <div className="space-y-2">
          {pendingItems.length > 0 ? (
              ...
              <div key={i} className="flex items-center gap-2">
                <button 
                  ...
                  className={`flex-1 flex items-center gap-3 p-4 rounded-2xl border transition-all ${
                    ...
                    : 'bg-brand-card border-white/[0.05] shadow-sm active:scale-[0.98]'
                  }`}
                >
                ...
                <button 
                    ...
                    className="p-3 text-brand-muted/20 hover:text-brand-danger transition-all active:scale-90"
                >
                  <X size={20}/>
                </button>
              </div>
            ...
            <div className="py-10 text-center border-2 border-dashed border-white/[0.02] rounded-[2rem]">
                <p className="text-[10px] font-black text-brand-muted uppercase tracking-[0.3em] opacity-20">
                    Tu lista está vacía
                </p>
            </div>
          )}
