...
import { Check, X, Store, Calendar, Euro, Link, Loader2, AlertCircle, Sparkles, Search, Plus } from 'lucide-react';
    ...
    <div className="modal-content-full !pb-6 flex flex-col animate-in slide-in-from-bottom duration-500 no-scrollbar">
                ...
                <div key={i} className={`flex flex-col transition-all duration-300 rounded-2xl border ${isExpanded ? 'bg-brand-primary/5 border-brand-primary/30 p-3' : 'bg-white/[0.02] border-white/[0.04] p-2'}`}>
                  <button onClick={() => toggleExpand(i)} className="flex items-center gap-3 text-left w-full">
                    ...
                    <div className="flex-1 overflow-hidden">
                      <div className="flex items-center gap-2 overflow-hidden">
                        <p className={`text-[11px] font-black uppercase truncate leading-tight ${hasAlias ? 'text-brand-accent' : 'text-white'}`}>{p.nombre_base}</p>
                  ...
                  {isExpanded && (
                    <div className="mt-3 pt-3 border-t border-brand-primary/10 animate-in slide-in-from-top-2 space-y-3">
                      <p className="text-[8px] font-black uppercase text-brand-primary opacity-60">Vincular con tu lista:</p>
                      <div className="flex flex-wrap gap-1.5">
                        ...
                        {showExtraSearchIdx !== i ? (
                            ...
                            <div className="w-full space-y-2 animate-in zoom-in-95">
                                <input autoFocus value={extraSearchTerm} onChange={(e) => handleExtraSearch(e.target.value)} placeholder="Buscar..." className="w-full bg-brand-bg/50 border border-brand-primary/30 p-2 rounded-xl text-[10px] uppercase font-bold outline-none" />
                                <div className="flex flex-wrap gap-1.5 max-h-32 overflow-y-auto no-scrollbar">
                                    {extraSuggestions.map((s, sIdx) => (
                                        <button key={sIdx} onClick={() => setAlias(i, s.nombre_base)} className="px-2 py-1 bg-brand-primary/20 rounded-lg text-[8px] font-black text-white uppercase">{s.nombre_base}</button>
                                    ))}
                                </div>
                            </div>
                        )}
      ...
      <div className="fixed bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-brand-bg via-brand-bg to-transparent">
        <div className="grid grid-cols-2 gap-3">
            <button onClick={onCancel} className="btn-secondary !bg-brand-danger/5 border-none text-brand-danger !lowercase !text-[10px] font-black">descartar</button>
            <button onClick={() => onSave(pendingGasto)} disabled={loading} className="btn-primary !py-4 shadow-xl !bg-brand-success text-brand-bg">
            {loading ? <Loader2 className="animate-spin" /> : <div className="flex items-center gap-2"><Check size={20} strokeWidth={4} /><span className="text-[11px] font-black tracking-widest uppercase">Guardar Compra</span></div>}
            </button>
        </div>
      </div>
    </div>
