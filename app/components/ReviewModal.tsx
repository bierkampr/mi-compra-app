...
import { Check, X, Store, Link, Loader2, Sparkles, Search, CheckCircle2 } from 'lucide-react';
...
const ReviewModal: React.FC<ReviewModalProps> = ({ 
  ...
  // Agrupar productos al cargar si no están agrupados
  useEffect(() => {
    ...
    <div className="modal-content-full !pb-6 flex flex-col animate-in slide-in-from-bottom duration-500 no-scrollbar">
          ...
          <div className="col-span-12 relative">
            ...
            <div className="relative">
              <input 
                ...
                className="input-premium !py-3 !text-sm uppercase" 
              />
          ...
          {isManualExpanded && (
            <div className="card-premium !p-3 bg-brand-primary/5 border-brand-primary/20 animate-in zoom-in-95 mb-4">
              <div className="flex gap-2 mb-2">
                <input 
                    placeholder={txt('review.placeholder_prod')} 
                ...
                />
                <input 
                    placeholder={txt('review.placeholder_price')} 
                    type="number" 
              ...
              <button onClick={addManualItem} className="btn-primary !py-2 !text-[9px]">
                {txt('review.manual_ok')}
              </button>
            </div>
          )}
                        ...
                        {showExtraSearchIdx !== i ? (
                            ...
                            <div className="w-full space-y-2 animate-in zoom-in-95">
                                <input 
                                    ...
                                    placeholder={txt('review.search_placeholder')} 
                                    className="w-full bg-brand-bg/50 border border-brand-primary/30 p-2 rounded-xl text-[10px] uppercase font-bold outline-none" 
                                />
      ...
      <div className="fixed bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-brand-bg via-brand-bg to-transparent">
        <div className="grid grid-cols-2 gap-3">
            <button 
                onClick={onCancel} 
                className="btn-secondary !bg-brand-danger/5 border-none text-brand-danger !lowercase !text-[10px] font-black"
            >
            ...
            {loading ? <Loader2 className="animate-spin" /> : (
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-[11px] font-black tracking-widest uppercase">{txt('review.save_btn')}</span>
                </div>
            )}
            </button>
        </div>
