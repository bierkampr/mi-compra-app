...
ScannerView.Capture = ({ tempPhotos, setTempPhotos, loading, startAnalysis, db, setShowListDialog, showListDialog, onCancel, txt, activeTab, clientId }) => {
  ...
  // Función para solicitar autorización incremental de IA
  const requestAiPermission = () => {
    ...
    const client = window.google.accounts.oauth2.initCodeClient({
      ...
      callback: async (response: any) => {
        if (response.code) {
          try {
            // Actualizamos los tokens en el servidor con el nuevo scope
  ...
  return (
    <div className="fixed inset-0 bg-brand-bg z-[2000] flex flex-col overflow-hidden animate-in slide-in-from-bottom-full duration-500">
      ...
      {/* VISTA DE PREPARACIÓN O BLOQUEO DE IA */}
      {!showCamera && (
        <div className="flex-1 flex flex-col p-6 overflow-y-auto no-scrollbar">
          <div className="flex justify-between items-center mb-8">
              <button
                ...
                className="btn-icon !bg-white/5 border-none cursor-pointer z-50 pointer-events-auto"
